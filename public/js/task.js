var Task = function(description, date, id, completion){
  ...
  this.createListItem = function () {
    ...
    if (this.completion === true){
      $('.complete-list').append(listItem).children(':last').hide().fadeIn(500);
      $('#' + this.task_id + ' .completion-status').attr("checked", true);
    }
    else {
